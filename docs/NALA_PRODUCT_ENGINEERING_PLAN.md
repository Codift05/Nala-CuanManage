...
| Evaluasi receipt extraction | 🟡 | Baseline sintetis: 30/30 draft, nominal/merchant 100%, kategori 83,33%, p95 2.294 ms; dataset nyata tetap diperlukan |
...
- [x] Simpan histori skor untuk evaluasi perubahan
...
| Contract | AI Coach memiliki serializer backend dan parser Flutter yang mengunci envelope sukses/fallback serta draft aman pada suite terpisah | Perluas ke auth, transaksi, wallet, budget, dan Habit Score |
...
| 1 Agustus 2026 | Welcome auth memakai identitas visual NALA | Hero kota pesisir orisinal dan koin bergerak memakai animasi native 4 detik; shortcut fitur, CTA Masuk, dan Daftar disusun ulang tanpa menyalin identitas aplikasi referensi |
...
4. **M4.3 — Context-Aware AI Coach**
   ...
   - Corpus adversarial, security gate, dan integration flow saran-draft-batal-
     konfirmasi selesai di codebase; hasil runner Linux menunggu bukti CI karena
     workstation lokal belum memiliki `libsecret` development package.
   - Contract response backend/Flutter selesai; berikutnya evaluasi model live
     yang terpisah dari CI dengan prompt, model, latency, hasil, dan biaya yang
     dapat ditelusuri.
...
melalui pilot. Sambil menunggu data primer, coding berlanjut ke **M4.3
Context-Aware AI Coach**. Confidence model hanya dipakai sebagai sinyal review,
bukan bukti akurasi.
